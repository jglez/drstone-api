    ...
    return (
      <div className="snippet-container return-container">
        <div className="return-data">
          {syntaxHighlight(JSON.stringify(_props.data, null, "\t")).split(/\n/).map((line, lineIndex) => {
            return (
              <div className="line" key={`line-${lineIndex}`}>
                {line.split(/\t/).map((spacedLine, spacingIndex) => {
                  return (
                    <div
                      key={`spacing-${lineIndex}-${spacingIndex}`}
                      style={{ marginRight: "35px" }}
                      dangerouslySetInnerHTML={{ __html: spacedLine }}
                    />
                  );
                })}
              </div>
